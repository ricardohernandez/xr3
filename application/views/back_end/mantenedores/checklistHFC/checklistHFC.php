...
<script type="text/javascript">
  ...
  /*****DATATABLE*****/   
    var tb_hfc = $('#tb_hfc').DataTable({
       ...
       "ajax": {
          "url":"<?php echo base_url();?>listaMantChecklistHFC",
          "dataSrc": function (json) {
            $(".btn_filtro_hfc").html('<i class="fa fa-cog fa-1x"></i><span class="sr-only"></span> Filtrar');
            $(".btn_filtro_hfc").prop("disabled" , false);
            return json;
          },       
          data: function(param){
       ...
       "columns": [
          {
           "class":"centered center margen-td","data": function(row,type,val,meta){
              btn='<center><a data-toggle="modal" href="#modal_hfc" data-hash_herramienta="'+row.hash_herramienta+'" data-placement="top" data-toggle="tooltip" title="Modificar" class="fa fa-edit btn_modificar_hfc"></a>';
          ...
          { "data": "tipo" ,"class":"margen-td centered"},
          {
            "class":"centered","data": function(row,type,val,meta){
              ...
              if(row.imagen!=""){
                html='<center><a target="_blank" href="./archivos/fotos_checklist/hfc/'+row.imagen+'" class="foto_icon" title="Imagen"><i class="fa fa-camera"></i></a>';
              }else{
                html="";
              }
              return html;
      ...
      $(document).on('keyup paste', '#buscador_hfc', function() {
        tb_hfc.search($(this).val().trim()).draw();
      });
      ...
      setTimeout( function () {
        var tb_hfc = $.fn.dataTable.fnTables(true);
        if ( tb_hfc.length > 0 ) {
            $(tb_hfc).dataTable().fnAdjustColumnSizing();
      }}, 200 ); 
      ...
      $(document).off('click', '.btn_filtro_hfc').on('click', '.btn_filtro_hfc',function(event) {
	      event.preventDefault();
	       $(this).prop("disabled" , true);
	       $(".btn_filtro_hfc").html('<i class="fa fa-cog fa-spin fa-1x fa-fw"></i><span class="sr-only"></span> Filtrando');
	       tb_hfc.ajax.reload();
	  });
  ...
  /*********INGRESO************/
    ...
    $(document).off('click', '.btn_nuevo_hfc').on('click', '.btn_nuevo_hfc',function(event) {
        ...
        $('#formMantChecklistHFC')[0].reset();
        $("#hash_herramienta").val("");
        $("#formMantChecklistHFC input,#formMantChecklistHFC select,#formMantChecklistHFC button,#formMantChecklistHFC").prop("disabled", false);
    });     


    $(document).off('submit', '#formMantChecklistHFC').on('submit', '#formMantChecklistHFC',function(event) {
      var url="<?php echo base_url()?>";
      var formElement = document.querySelector("#formMantChecklistHFC");
      var formData = new FormData(formElement);
        $.ajax({
            url: $('#formMantChecklistHFC').attr('action')+"?"+$.now(),  
            type: 'POST',
            data: formData,
            cache: false,
            processData: false,
            ...
            beforeSend:function(){
              ...
              $("#formMantChecklistHFC input,#formMantChecklistHFC select,#formMantChecklistHFC button,#formMantChecklistHFC").prop("disabled", true);
            },
   ...
   $(document).off('click', '.btn_modificar_hfc').on('click', '.btn_modificar_hfc',function(event) {
      ...
      $.ajax({
        url: "getDataMantChecklistHFC"+"?"+$.now(),  
        type: 'POST',
        cache: false,
        tryCount : 0,
        retryLimit : 3,
    ...
    $(document).off('click', '.borrar_herramienta').on('click', '.borrar_herramienta',function(event) {
        var hash_herramienta=$(this).attr("data-hash_herramienta");
          if(confirm("¿Esta seguro que desea eliminar este registro?")){
            $.post('eliminaMantChecklistHFC'+"?"+$.now(),{"hash_herramienta": hash_herramienta}, function(data) {
              if(data.res=="ok"){
                $.notify(data.msg, {
                  className:'success',
                  globalPosition: 'top right'
    ...
    $(document).off('click', '.btn_excel_hfc').on('click', '.btn_excel_hfc',function(event) {
      event.preventDefault();
      var tipo = $("#tipo_f").val()
      tipo = tipo=="" ? "-" : tipo;
      window.location="excelMantChecklistHFC/"+tipo;
    });
...
<!-- FILTROS -->
    ...
    <!--   <div class="col-xs-6 col-sm-6 col-md-1 col-lg-1 no-padding">  
      ...
      <div class="col-6 col-lg-1">
        <div class="form-group">
         <button type="button" class="btn-block btn btn-sm btn-primary btn_filtro_hfc btn_xr3">
      ...
      <div class="col-6 col-lg-1">  
        <div class="form-group">
         <button type="button"  class="btn-block btn btn-sm btn-primary btn_excel_hfc btn_xr3">
...
<!-- LISTADO -->
  ...
  <div class="row">
    <div class="col-lg-12">
      <table id="tb_hfc" class="table table-striped table-hover table-bordered dt-responsive nowrap" style="width:100%">
        <thead>
          <tr>    
...
<!--  FORMULARIO-->
  <div id="modal_hfc" data-backdrop="static"  data-keyboard="false"   class="modal fade">
   <?php echo form_open_multipart("formMantChecklistHFC",array("id"=>"formMantChecklistHFC","class"=>"formMantChecklistHFC"))?>
      ...
      <div class="modal-content">
          ...
          <div class="col-xs-12 col-sm-12 col-lg-8 offset-lg-2 mt-0">
	          <div class="form-row">
	            <div class="col-9 col-lg-6">
	                <button type="submit" class="btn-block btn btn-sm btn-primary btn_guardar_hfc">
	                 <i class="fa fa-save"></i> Guardar
	                </button>
	            </div>
	            <div class="col-3 col-lg-6">
	              <button class="btn-block btn btn-sm btn-secondary cierra_modal_hfc" data-dismiss="modal" aria-hidden="true">
        ...
        <div class="modal-body">
         <!--  <button type="button" title="Cerrar Ventana" class="close" data-dismiss="modal" aria-hidden="true">X</button> -->
          ...
          <legend class="form-ing-border">Registro checklist hfc </legend>
