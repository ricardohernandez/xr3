...
<script type="text/javascript">
	...
	google.charts.setOnLoadCallback(graficoFallosFTTH);

	function graficoFallosFTTH(){
		...
		$.ajax({
	        url: base_url+"graficoFallosFTTH"+"?"+$.now(),  
	        type: 'POST',
	        data:{desde:desde,hasta:hasta,trabajador:trabajador,auditor:auditor},
	        dataType:"json",
	        success: function (json) {
				  var graficoFallosFTTH = google.visualization.arrayToDataTable(json);
       		graficoFallosFTTH.sort([{column: 4, desc: false}])
       		var options = {
		        isStacked: true,
	            width: "100%",
	            height: 460,
			    ...
			    var chart = new google.visualization.ComboChart(document.getElementById('graficoFallosFTTH'));
			    chart.draw(graficoFallosFTTH, options);
    	    }
        })
	  }

	
    $.getJSON(base_url + "listaTrabajadoresFTTH" , function(data) {
	      response = data;
		}).done(function() {
