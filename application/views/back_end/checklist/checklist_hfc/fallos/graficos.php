...
<script type="text/javascript">
	...
	google.charts.setOnLoadCallback(graficoFallosHFC);

	function graficoFallosHFC(){
		...
		$.ajax({
	        url: base_url+"graficoFallosHFC"+"?"+$.now(),  
	        type: 'POST',
	        data:{desde:desde,hasta:hasta,trabajador:trabajador,auditor:auditor},
	        dataType:"json",
	        success: function (json) {
				  var graficoFallosHFC = google.visualization.arrayToDataTable(json);
       		graficoFallosHFC.sort([{column: 4, desc: false}])
       		var options = {
		        isStacked: true,
	            width: "100%",
	            height: 460,
			    ...
			    var chart = new google.visualization.ComboChart(document.getElementById('graficoFallosHFC'));
			    chart.draw(graficoFallosHFC, options);
    	    }
        })
	  }

	
    $.getJSON(base_url + "listaTrabajadoresHFC" , function(data) {
	      response = data;
		}).done(function() {
