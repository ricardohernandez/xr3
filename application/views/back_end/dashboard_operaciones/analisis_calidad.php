...
<script>
  ...
  $.ajax({
      ...
      success: function (response) {
        $("#load").hide()
        $(".body").fadeIn(500)
        crearGraficooAnalisisCalidad(response);
      },
  ...
  function crearGraficooAnalisisCalidad(data) {
    console.log(data)
  ...
  $(document).off('change', '#mes_inicio_cal,#mes_termino_cal').on('change', '#mes_inicio_cal,#mes_termino_cal', function (event) {
    cargarGraficoAnalisisCalidad();
  });
