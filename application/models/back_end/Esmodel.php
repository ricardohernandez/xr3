...
class Esmodel extends CI_Model {
	...
	/**********Es*************/
		...
		public function getEsList($desde,$hasta,$estado,$tecnico,$supervisor){
			...
			$this->db->join('es_mantenedor_requerimientos req_mant', 'req_mant.id = r.id_requerimiento', 'left');

			$this->db->join('es_tipos req_mant_t', 'req_mant_t.id = req_mant.id_tipo', 'left');

			$this->db->join('es_cierres cierre', 'cierre.id = r.id_cierre', 'left');

			$this->db->join('usuarios_areas z', 'z.id = r.id_zona', 'left');
			...
			if ($tecnico != "") {
				...
				$this->db->where('r.id_tecnico', $tecnico);

			}
			if ($supervisor != "") {

				$this->db->where('ust.id_jefe', $supervisor);
		...
		public function getDataEs($hash){
			...
			$this->db->select('r.*,
				...
				ust.correo_empresa as correo_tecnico,
		...
		public function getEsListVencidas(){
		...
		public function eliminaEs($hash){

			$this->db->where('sha1(id)', $hash);

			if($this ->db->delete('es')){
		...
		public function listaCierres(){

			$this->db->order_by('cierre', 'asc');

			$res=$this->db->get('es_cierres');
		...
		public function listaRequerimientos($tipo){
			...
			$res=$this->db->get("es_mantenedor_requerimientos");
		...
		public function listaResponsables(){
			...
			$this->db->join('es_mantenedor_requerimientos rm', 'rm.id_responsable1 = u.id OR rm.id_responsable2 = u.id', 'inner');
		...
		public function getEstadoEs($hash){
		...
		public function getValidadorRealEs($hash){
		...
		public function getUsuarioAsignadoEs($hash){
	...
	/**********MANTENEDOR REQUERIMIENTOS************/
		...
		public function getMantenedorReqList($estado){
			...
			$this->db->join('es_tipos rt', 'rt.id = r.id_tipo', 'left');
			...
			$res = $this->db->get('es_mantenedor_requerimientos as r');
		...
		public function formActualizarMantenedorReq($id,$data){
			...
			if($this->db->update('es_mantenedor_requerimientos', $data)){
		...
		public function formIngresoMantenedorReq($data){
			...
			if($this->db->insert('es_mantenedor_requerimientos', $data)){
		...
		public function eliminaMantenedorReq($hash){
			...
			if($this ->db->delete('es_mantenedor_requerimientos')){
		...
		/*******TIPO**************/
			...
			public function formActualizarMantenedorReqTipo($id,$data){
				...
				if($this->db->update('es_tipos', $data)){
			...
			public function formIngresoMantenedorReqTipo($data){
				...
				if($this->db->insert('es_tipos', $data)){
