	...
	/**********DATAS************/
		...
		public function getDatasList(){
			$this->db->select("sha1(c.id) as hash_id,
					...
					CONCAT(u.nombres,' ',u.apellidos)  'digitador'");
			$this->db->join('usuarios u', 'u.id = c.id_digitador', 'left');
			$res=$this->db->get('documentacion_datas_mandante as c');
			return $res->result_array();
		}

		
		public function getDataRegistroDatas($hash){
			$this->db->select("sha1(c.id) as hash_id,
					c.*");
			$this->db->where('sha1(c.id)', $hash);
			$res=$this->db->get('documentacion_datas_mandante as c');
		...
		public function formActualizarDatas($id,$data){
			$this->db->where('sha1(id)', $id);
		    if($this->db->update('documentacion_datas_mandante', $data)){
		    	return TRUE;
		    }
		    return FALSE;
		}

		public function formIngresoDatas($data){
			if($this->db->insert('documentacion_datas_mandante', $data)){
				return $this->db->insert_id();
			}
			return FALSE;
		} 
		
		public function eliminaDatas($hash){
			$this->db->where('sha1(id)', $hash);
			if($this ->db->delete('documentacion_datas_mandante')){
			  	return TRUE;
			}
			return FALSE;
		}
