...
class Prevencion_modulos extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model("back_end/Prevencion_modulosmodel");
	...
	public function visitas($modulo,$id_aplicacion){
    	...
    	$this->Prevencion_modulosmodel->insertarVisita($data);
	}
	...
	/***********PREVENCION MODULOS*****************/
		...
		public function vistaPrevencionModulos(){
			...
			if($this->input->is_ajax_request()){
				...
				$this->load->view('back_end/prevencion_modulos/modulos',$datos);
			}
		}
		...
		public function getList(){
			...
			foreach ($lista as &$key) {
				...
				$key['descripcion'] = preg_replace('/\s+/', ' ', $key['descripcion']);
				$key['descripcion'] = trim($key['descripcion']);
			}
		...
		public function formIngreso(){
			if($this->input->is_ajax_request()){
				...
				$link3=@$_FILES["link3"]["name"];
	   			...
	   			if ($this->form_validation->run("formIngresoPrevencionModulos") == FALSE){
			    	...
			    	if($hash==FALSE){
						...
						$insert_id=$this->Prevencion_modulosmodel->formIngreso($data);
						if($insert_id!=FALSE){
						...
						'descripcion'=>$descripcion,
						 ...
						 if($archivo_principal!=""){
							...
							$imagen=$this->Prevencion_modulosmodel->getImagen($hash);
							if($imagen!=""){
						...
						if($this->Prevencion_modulosmodel->formActualizar($hash,$data)){
		...
		public function eliminar(){
			...
			$data=$this->Prevencion_modulosmodel->getData($hash);
			foreach($data as $key){
				if($key["archivo"]!=""){
			...
			if($this->Prevencion_modulosmodel->eliminar($hash)){
