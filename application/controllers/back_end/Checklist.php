...
class Checklist extends CI_Controller {
	...
	/*********REGISTRO OTS************/
		...
		public function getDataOTS(){
			if($this->input->is_ajax_request()){
				$this->checkLogin();
				$hash=$this->security->xss_clean(strip_tags($this->input->post("hash")));
				$data=$this->Checklistmodel->getDataOTS($hash);
		...
		public function excel_checklist(){
			...
			if(!$data){
				...
				<h3>Reporte envio OTS  <?php echo date("d-m-Y",strtotime($desde)); ?> - <?php echo date("d-m-Y",strtotime($hasta)); ?></h3>
