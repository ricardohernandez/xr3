...
class ChecklistHFC extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model("back_end/mantenedores/ChecklistHFCmodel");
	...
	public function index(){
		$this->visitas("Mantenedor Checklist HFC");
    	$this->acceso();
	    $datos = array(
	        'titulo' => "Mantenedor Checklist HFC",
	        'contenido' => "mantenedores/checklistHFC/inicio",
	        'perfiles' => $this->Iniciomodel->listaPerfiles(),
		);  
		$this->load->view('plantillas/plantilla_back_end',$datos);
	}

    public function vistaMantChecklistHFC(){
		...
		if($this->input->is_ajax_request()){
			...
			$datos=array(	
		        ...
		        'fecha_hoy' => $fecha_hoy,
		        'tipos' => $this->ChecklistHFCmodel->getTipos()
		    );
			$this->load->view('back_end/mantenedores/checklistHFC/checklistHFC',$datos);
		}
	}

	public function listaMantChecklistHFC(){
		$tipo=$this->security->xss_clean(strip_tags($this->input->get_post("tipo")));
		echo json_encode($this->ChecklistHFCmodel->listaMantChecklistHFC($tipo));
	}

	public function formMantChecklistHFC(){
		if($this->input->is_ajax_request()){
			$this->checkLogin();
			...
			if ($this->form_validation->run("formMantChecklistHFC") == FALSE){
				...
				if($hash_herramienta==""){
					...
					$id=$this->ChecklistHFCmodel->formMantChecklistHFC($data);
					if($id!=FALSE){
					...
					if($this->ChecklistHFCmodel->actualizarMantChecklistHFC($hash_herramienta,$data)){
						echo json_encode(array('res'=>"ok", 'msg' => MOD_MSG));exit;
					}else{
						echo json_encode(array('res'=>"ok",  'msg' => MOD_MSG));exit;
					}
	...
	public function procesaArchivoH($file,$titulo){
		...
		$config['upload_path'] = './archivos/fotos_checklist/hfc';
	...
	public function eliminaMantChecklistHFC(){
		$hash=$this->security->xss_clean(strip_tags($this->input->post("hash_herramienta")));
	    if($this->ChecklistHFCmodel->eliminaMantChecklistHFC($hash)){
	...
	public function getDataMantChecklistHFC(){
		if($this->input->is_ajax_request()){
			$this->checkLogin();
			$hash_herramienta=$this->security->xss_clean(strip_tags($this->input->post("hash_herramienta")));
			$data=$this->ChecklistHFCmodel->getDataMantChecklistHFC($hash_herramienta);
	...
	public function excelMantChecklistHFC(){
		$tipo=$this->uri->segment(2);
		if($tipo=="-"){$tipo="";}

		$data=$this->ChecklistHFCmodel->listaMantChecklistHFC($tipo);

		if(!$data){
			return FALSE;
		}else{

			$nombre="reporte-checklist_hfc.xls";
