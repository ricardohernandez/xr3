...
class Herramientas extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model("back_end/mantenedores/Herramientasmodel");
	...
	public function visitas($modulo){
    	...
    	$this->Herramientasmodel->insertarVisita($data);
	}
	...
	public function index(){
		$this->visitas("Mantenedor herramientas");
    	$this->acceso();
	    $datos = array(
	        'titulo' => "Mantenedor herramientas",
	        'contenido' => "mantenedores/herramientas/inicio",
    ...
    public function vistaHerramientas(){
		...
		if($this->input->is_ajax_request()){
			...
			$datos=array(	
		        ...
		        'fecha_hoy' => $fecha_hoy,
		        'tipos' => $this->Herramientasmodel->getTipos()
		    );

			$this->load->view('back_end/mantenedores/herramientas/herramientas',$datos);
		}
	}

	public function listaHerramientas(){
		$tipo=$this->security->xss_clean(strip_tags($this->input->get_post("tipo")));
		echo json_encode($this->Herramientasmodel->listaHerramientas($tipo));
	}
		...
		if($this->input->is_ajax_request()){
			...
			if ($this->form_validation->run("formHerramientas") == FALSE){
				...
				if($hash_herramienta==""){
					...
					$id=$this->Herramientasmodel->formHerramientas($data);
					if($id!=FALSE){
					...
					if($this->Herramientasmodel->actualizarHerramientas($hash_herramienta,$data)){
						echo json_encode(array('res'=>"ok", 'msg' => MOD_MSG));exit;
					}else{
						echo json_encode(array('res'=>"ok",  'msg' => MOD_MSG));exit;
					}
	...
	public function getDataHerramientas(){
		if($this->input->is_ajax_request()){
			$this->checkLogin();
			$hash_herramienta=$this->security->xss_clean(strip_tags($this->input->post("hash_herramienta")));
			$data=$this->Herramientasmodel->getDataHerramientas($hash_herramienta);
	...
	public function excelHerramientas(){
		...
		$data=$this->Herramientasmodel->listaHerramientas($tipo);

		if(!$data){
			return FALSE;
		}else{

			$nombre="reporte-herramientas.xls";
